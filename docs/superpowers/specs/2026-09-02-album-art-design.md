...
With cover on, transport is **square cover | stacked chrome**. The cover starts on the library pane's left border column and is pinned to the top of the slot. Same fields as today (title, artist/album, seek, volume, toggles, signal path), wrapped in the right column. Cover keeps aspect (`Resize::Fit`). Typical cells make an 8-row square about 16 columns; real width comes from the picker font size. If font size is unknown (tests), use `width = 2 * height` columns.
