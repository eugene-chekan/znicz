...
inspector, devices, playlists, radio, and the queue drawer close with the
top-right **X**, or with `Esc`, `]`, or a clickable footer hint — not by
clicking outside. Footer hint segments are clickable and run that key action.
Click the library pane's right-border column to open the queue drawer only
(the **X**, `Esc`, or `]` close it). Library search and playlist/radio typing
prompts still cancel on an outside click. Clicks on the transport and cover do
nothing. Terminals that never send mouse events keep working from the keyboard.
`?` stays keyboard-only.
...
stacked chrome on the right, with one empty cell between them. The cover
