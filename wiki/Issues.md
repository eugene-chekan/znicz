...
Product phase **5** (album art) is done. Phase **6** (MusicBrainz) and **later
radio** (HLS, PLS, XSPF) are on the
...
- **Fixed:** 2026-09-03
- **Component:** `znicz-tui`
- **Status:** **Fixed** in 0.4.5 (select and wheel in 0.4.4; **X** and clickable
  footer hints in 0.4.5)

Left click selects a visible list row. Wheel steps the focused list. Overlays
close with the top-right **X**, `Esc`, or a clickable footer hint — not outside
click. The queue drawer closes with **X**, global `]`, or footer `] close` /
`] queue`; `Esc` closes it only when queue focus is on the drawer. The library
right-border column opens the queue drawer only. Typing prompts still cancel on
outside click. Footer hint segments are clickable. Capture is on for the life of
the TUI. Drag-to-reorder and seek-bar clicks stay out.
